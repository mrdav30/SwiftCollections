...
public class OctreeNode<T> where T : IOctreeItem
{
    ...
    /// On the other hand, setting it too low could result in an excessively deep octree,
    /// which could also hurt performance and consume more memory. 
    ...
    public SwiftList<T> MyObjects { get; private set; }
    ...
    private void Subdivide()
    {
        ...
        SwiftList<T> oldObjects = MyObjects;
        MyObjects.Clear();
        for (int i = 0; i < oldObjects.Count; i++)
            Insert(oldObjects[i]); // Re-insert the items into the subdivided node
    }

    public void Query(Vector3d position, Fixed64 radius, ref SwiftList<T> result)
    {
