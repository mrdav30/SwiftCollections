...
{
    private SwiftDictionary<int, SwiftList<T>> _buckets;
    private Fixed64 _size;

    public SpatialHash(Fixed64 size)
    {
        _buckets = new SwiftDictionary<int, SwiftList<T>>();
        _size = size;
    }
    ...
    public void Insert(T item)
    {
        ...
        if (!_buckets.TryGetValue(hash, out SwiftList<T> bucket))
        {
            bucket = new SwiftList<T>();
            _buckets[hash] = bucket;
        }
        bucket.Add(item);
    }

    public void Query(Vector3d position, ref SwiftList<T> result)
    {
        int originalHash = Hash(position);
        SwiftList<int> hashesToCheck = new SwiftList<int>() { originalHash };
