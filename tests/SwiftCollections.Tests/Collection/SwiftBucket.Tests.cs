...
public class SwiftBucketTests
{
    ...
    public void SwiftBucket_Serialization_RoundTripMaintainsData()
    {
        ...
        Assert.Equal(originalBucket.PeakCount, deserializedBucket.PeakCount);
    ...
    {
        var originalValue = new SwiftBucket<int>();
        for (int i = 0; i < 100; i++)
            originalValue.Add(i);

        byte[] bytes = MemoryPackSerializer.Serialize(originalValue);
        SwiftBucket<int> deserializedValue = MemoryPackSerializer.Deserialize<SwiftBucket<int>>(bytes);
