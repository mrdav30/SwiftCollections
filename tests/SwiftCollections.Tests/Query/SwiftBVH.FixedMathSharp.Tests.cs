...
{
    public class SwiftBVHFixedMathSharpTests
    {
        [Fact]
        ...
        public void Query_NoIntersection_ReturnsEmpty()
        {
            ...
            var volume2 = new FixedBoundVolume(new Vector3d(10, 10, 10), new Vector3d(11, 11, 11));
        ...
        public void Query_OverlappingVolumes_ReturnsAllIntersectingKeys()
        {
            ...
            var queryVolume = new FixedBoundVolume(new Vector3d(0.25f, 0.25f, 0.25f), new Vector3d(1.25f, 1.25f, 1.25f));
            var results = new List<int>();
        ...
        public void Insert_LargeNumberOfVolumes_PerformsEfficiently()
        {
            ...
            for (int i = 0; i < numVolumes; i++)
            {
                ...
                var max = new Vector3d(
                    min.x + new Fixed64(random.NextDouble() * 10),
                    min.y + new Fixed64(random.NextDouble() * 10),
                    min.z + new Fixed64(random.NextDouble() * 10));
        ...
        public void BoundingVolume_Intersects_ReturnsCorrectly()
        {
            var volume1 = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(1, 1, 1));
            var volume2 = new FixedBoundVolume(new Vector3d(0.5f, 0.5f, 0.5f), new Vector3d(1.5f, 1.5f, 1.5f));
            var volume3 = new FixedBoundVolume(new Vector3d(2, 2, 2), new Vector3d(3, 3, 3));
        ...
        public void UpdateEntry_PropagatesChanges()
        {
            var bvh = new SwiftBVH<int>(10);
            var volume1 = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(1, 1, 1));
            var volume2 = new FixedBoundVolume(new Vector3d(2, 2, 2), new Vector3d(3, 3, 3));

            bvh.Insert(0, volume1);
            bvh.Insert(1, volume2);

            // Update volume1 to overlap with volume2
            var updatedVolume1 = new FixedBoundVolume(new Vector3d(1.5f, 1.5f, 1.5f), new Vector3d(2.5f, 2.5f, 2.5f));
            bvh.UpdateEntryBounds(0, updatedVolume1);

            var results = new List<int>();
            var queryVolume = new FixedBoundVolume(new Vector3d(2, 2, 2), new Vector3d(3, 3, 3));
            bvh.Query(queryVolume, results);
        ...
        public void TreeDepth_DoesNotGrowExponentially_Linear()
        {
            ...
            for (int i = 0; i < numVolumes; i++)
            {
                var volume = new FixedBoundVolume(new Vector3d(i, i, i), new Vector3d(i + 1, i + 1, i + 1));
        ...
        public void TreeDepth_DoesNotGrowExponentially_Random()
        {
            ...
            for (int i = 0; i < numVolumes; i++)
            {
                var volume = new FixedBoundVolume(
                    new Vector3d(random.Next(0, 100), random.Next(0, 100), random.Next(0, 100)),
                    new Vector3d(random.Next(1, 10), random.Next(1, 10), random.Next(1, 10))
                );
                bvh.Insert(Guid.NewGuid().ToString(), volume);
            }
        ...
        public void StressTest_ResizingIntegrity()
        {
            ...
            var queryVolume = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(10000, 10000, 10000));
            var results = new List<int>();
            bvh.Query(queryVolume, results);
            ...
            for (int t = 0; t < numThreads; t++)
            {
                ...
                threads.Add(new Thread(() =>
                {
                    for (int i = 0; i < volumesPerThread; i++)
                    {
                        ...
                        var volume = new FixedBoundVolume(new Vector3d(id, id, id), new Vector3d(id + 1, id + 1, id + 1));
                        bvh.Insert(id, volume);
                        bvh.UpdateEntryBounds(id, new FixedBoundVolume(new Vector3d(id - 1, id - 1, id - 1), new Vector3d(id + 2, id + 2, id + 2)));
                    }
                }));
            }
        ...
        public void MultiThreaded_HighConcurrency()
        {
            ...
            var results = new List<int>();
            var queryVolume = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(500, 500, 500));
            bvh.Query(queryVolume, results);
        ...
        {
            var bvh = new SwiftBVH<int>(10);
            var queryVolume = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(1, 1, 1));
        ...
        public void RapidInsertionsAndDeletions_MaintainTreeIntegrity()
        {
            ...
            var results = new List<int>();
            var queryVolume = new FixedBoundVolume(new Vector3d(0, 0, 0), new Vector3d(50, 50, 50));
            bvh.Query(queryVolume, results);
